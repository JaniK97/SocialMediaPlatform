...
class Message {
    ...
    // hae & palauta viimeisin käyttäjä
	...
	public function getLatestMessage($userLoggedIn, $user2) {
		...
		$start_date = new DateTime($row['date']); //Postauksen aika
		$end_date = new DateTime($date_time_now); //Tämän hetkinen aika
	...
	public function getConvosDropdown($data, $limit) {
		...
		$num_iterations = 0; //tarkisttettujen viestien lukumäärä
		$count = 1; //postattujen viestien määrä

		foreach($convos as $username) {
// jos ei ole saavuttanut start pointtia vielä (jatka)
			if($num_iterations++ < $start)
				continue;
// jos määrä ylittyy ladattavien viestien määrässä break (leave the loop)
			if($count > $limit)
				break;
			else 
				$count++;
    ...
    // hae lukemattomien viestien määrä
